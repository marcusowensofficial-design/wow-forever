...
local function GetRPNameColor(unit)
    ...
    if not guid or issecretvalue(guid) or issecretvalue(UnitName(unit)) then return end
...
local function UpdateHealthColor(frame, unit)
    ...
    if BBF.GetMarcusHealthBarColor then
        local mColor = BBF.GetMarcusHealthBarColor(unit)
        if mColor then
            frame:SetStatusBarDesaturated(true)
            frame:SetStatusBarColor(mColor.r, mColor.g, mColor.b, mColor.a or 1)
            return
        end
    end
