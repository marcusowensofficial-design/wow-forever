...
function BBF.UpdateUserAuraSettings()
    ...
    S.scale = db.marcusTargetBuffsScale or db.targetAndFocusAuraScale or 1
...
local function GetHostSizes(host)
    local isTargetSquare = (host.key == "target" and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsSquare ~= false))
    if host.isPlayer or isTargetSquare then
        ...
        local hideDur = isTargetSquare and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsHideDuration) or false
        if S.playerDurationOnIcon or hideDur then
...
local function GetMaxLineSize(host, sizes, primarySpacing)
    ...
    elseif host.key == "target" and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsSquare ~= false) then
        local perRow = math.max((BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsPerRow) or 8, 1)
        return perRow * PLAYER_AURA_ICON + (perRow - 1) * primarySpacing
    end
...
local function BuildStyle(tier, sizes, isPlayer, cfg, into)
    ...
    local isTargetSquare = sizes and sizes.isTargetSquare
    local playerLike = isPlayer or isTargetSquare
    ...
    if playerLike then
        ...
        local hideDur = isTargetSquare and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsHideDuration) or false
        showTimerText = not hideDur and (isPlayer and (S.playerDurations and true or false) or true)
        legacyBorder = false
        clickthrough = isPlayer and (S.clickthroughPlayerAuras and true or false) or true
    else
    ...
    t.tier = tier
    t.size = size
    t.iconSize = playerLike and PLAYER_AURA_ICON or size
    ...
    t.durationCurve = (isPlayer or isTargetSquare) and GetPlayerDurationCurve() or nil
    t.durationYOffset = GetDurationYOffset(playerLike, cfg, durationUnderIcon)
    t.stackScale = S.stackScale
    t.showTimerText = showTimerText
    t.hideLongTimers = (not playerLike and S.hideLongTimers) and true or false
    t.isPlayer = playerLike
    t.playerCooldown = isPlayer and S.playerCooldown or false
    ...
    t.borderInset = (playerLike and (not cfg.harmful or t.removeDebuffBorder))
        and PLAYER_BUFF_BORDER_INSET or BORDER_INSET
    t.legacyBorder = legacyBorder
    t.showDispelType = cfg.showDispelType
    t.drawBorder = (playerLike or S.pixelBorder or S.darkBorder) and true or false
    t.cropIcon = (playerLike or S.pixelBorder or S.darkBorder) and true or false
...
local function ContainerScale(host)
    if host.key == "target" and BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsScale then
        return BetterBlizzFramesDB.marcusTargetBuffsScale
    end
    local scale = host.scale or (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsScale) or S.scale or 1
...
local function ConfigureSpacer(host, harmful)
    ...
    spacer:SetScale(ContainerScale(host))
end
...
local function ConfigureContainer(host, container, harmful)
    ...
    container:SetScale(ContainerScale(host))
...
function BBF.AnchorAuraContainer(host)
    ...
    local isTargetSquare = (host.key == "target" and BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsSquare ~= false)
    local anchorTo
    if isTargetSquare then
        local tMana = TargetFrame.manabar
        ...
        local tHealth = TargetFrame.healthbar
        ...
        anchorTo = (tMana and tMana:IsShown() and tMana) or (tHealth and tHealth:IsShown() and tHealth) or (frameContainer and frameContainer.FrameTexture) or frame
    else
        anchorTo = (frameContainer and frameContainer.FrameTexture) or frame
    end

    local point, relPoint, startY, growth
    if buffsOnTop then
        point, relPoint = "BOTTOMLEFT", "TOPLEFT"
        startY = (isTargetSquare and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsYOffset or -4) or (AURA_MIRRORED_START_Y + S.offsetY))
        growth = AnchorUtil.FlowDirection.Up
    else
        point, relPoint = "TOPLEFT", "BOTTOMLEFT"
        startY = (isTargetSquare and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsYOffset or -4) or (AURA_START_Y + S.offsetY))
    ...
    local offsetX = isTargetSquare and (BetterBlizzFramesDB and BetterBlizzFramesDB.marcusTargetBuffsXOffset or 0) or S.offsetX
    local startX = (isTargetSquare and offsetX or (AURA_START_X + offsetX)) / scale
    startY = startY / scale
