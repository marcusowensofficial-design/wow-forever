...
local function guiMarcusCustomOptions()
    local guiMarcus = CreateFrame("Frame")
    guiMarcus.name = "Marcus Custom Options"
    guiMarcus.parent = BetterBlizzFrames.name
    local marcusCategory = Settings.RegisterCanvasLayoutSubcategory(BBF.category, guiMarcus, guiMarcus.name, guiMarcus.name)
    marcusCategory.ID = guiMarcus.name
    BBF.guiMarcus = guiMarcus.name
    BBF.category.marcusCategory = marcusCategory.ID
    CreateTitle(guiMarcus)

    local bgImg = guiMarcus:CreateTexture(nil, "BACKGROUND")
    bgImg:SetAtlas("professions-recipe-background")
    bgImg:SetPoint("CENTER", guiMarcus, "CENTER", -8, 4)
    bgImg:SetSize(680, 610)
    bgImg:SetAlpha(0.4)
    bgImg:SetVertexColor(0, 0, 0)

    local scrollFrame = CreateFrame("ScrollFrame", nil, guiMarcus, "UIPanelScrollFrameTemplate")
    scrollFrame:SetSize(700, 612)
    scrollFrame:SetPoint("CENTER", guiMarcus, "CENTER", -20, 3)

    local contentFrame = CreateFrame("Frame", nil, scrollFrame)
    contentFrame.name = guiMarcus.name
    contentFrame:SetSize(680, 1900)
    scrollFrame:SetScrollChild(contentFrame)

    local mainAnchor = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    mainAnchor:SetPoint("TOPLEFT", 30, -10)
    mainAnchor:SetText(" ")

    local headerText = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalHuge")
    headerText:SetPoint("TOPLEFT", mainAnchor, "BOTTOMLEFT", 0, 0)
    headerText:SetText("|cff00c0ffMarcus Custom Options|r")

local descText = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    descText:SetPoint("TOPLEFT", headerText, "BOTTOMLEFT", 0, -6)
    descText:SetText("Real-time combat health & resource text and health bar frame coloring for Player and Target frames.")

    -- Top Quick-Action Buttons (Save & Save/Reload)
    local topSaveBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    topSaveBtn:SetSize(140, 24)
    topSaveBtn:SetPoint("TOPRIGHT", contentFrame, "TOPRIGHT", -170, -14)
    topSaveBtn:SetText("Save Settings")
    local topSaveFont = topSaveBtn:GetFontString()
    if topSaveFont then
        topSaveFont:SetFont(fontSmall or GameFontNormalSmall:GetFont(), 11)
        topSaveFont:SetTextColor(0, 1, 0, 1)
    end

    local topReloadBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    topReloadBtn:SetSize(140, 24)
    topReloadBtn:SetPoint("TOPRIGHT", contentFrame, "TOPRIGHT", -20, -14)
    topReloadBtn:SetText("Save & Reload UI")
    local topReloadFont = topReloadBtn:GetFontString()
    if topReloadFont then
        topReloadFont:SetFont(fontSmall or GameFontNormalSmall:GetFont(), 11)
        topReloadFont:SetTextColor(1, 0.82, 0, 1)
    end

    ---------------------------------------------------------------------------
    -- SECTION 1: HEALTH & RESOURCE BAR TEXT
    ---------------------------------------------------------------------------
    local sec1Header = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    sec1Header:SetPoint("TOPLEFT", descText, "BOTTOMLEFT", 0, -22)
    sec1Header:SetText("|cffffd100HEALTH & RESOURCE BAR TEXT (420/420 100%)|r")

    local sec1Desc = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    sec1Desc:SetPoint("TOPLEFT", sec1Header, "BOTTOMLEFT", 0, -4)
    sec1Desc:SetText("Shows live numbers on Player & Target frames. Updates smoothly and instantly in combat.")

    local textFormatOptions = {
        "Both (420/420     100%)",
        "Health / Resource (420/420)",
        "Percentage (100%)",
        "Disabled",
    }

    -- Player Frame Column
    local playerTextHeader = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalMed2")
    playerTextHeader:SetPoint("TOPLEFT", sec1Desc, "BOTTOMLEFT", 10, -18)
    playerTextHeader:SetText("|cff00c0ffPlayer Frame Text|r")

    local pHpLabel = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    pHpLabel:SetPoint("TOPLEFT", playerTextHeader, "BOTTOMLEFT", 0, -10)
    pHpLabel:SetText("Health Bar Text:")

    local pHpDropdown = CreateSimpleDropdown("MarcusPlayerHealthTextDropdown", contentFrame, "", "marcusPlayerHealthText", textFormatOptions, function(val)
        BetterBlizzFramesDB.marcusPlayerHealthText = val
        if BBF.RefreshMarcusTextLayout then BBF.RefreshMarcusTextLayout() end
    end, { anchorFrame = pHpLabel, x = 0, y = -14 }, 210)

    local pResLabel = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    pResLabel:SetPoint("TOPLEFT", pHpDropdown, "BOTTOMLEFT", 0, -12)
    pResLabel:SetText("Mana / Resource Bar Text:")

    local pResDropdown = CreateSimpleDropdown("MarcusPlayerResourceTextDropdown", contentFrame, "", "marcusPlayerResourceText", textFormatOptions, function(val)
        BetterBlizzFramesDB.marcusPlayerResourceText = val
        if BBF.RefreshMarcusTextLayout then BBF.RefreshMarcusTextLayout() end
    end, { anchorFrame = pResLabel, x = 0, y = -14 }, 210)

    -- Target Frame Column
    local targetTextHeader = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalMed2")
    targetTextHeader:SetPoint("TOPLEFT", sec1Desc, "BOTTOMLEFT", 340, -18)
    targetTextHeader:SetText("|cff00c0ffTarget Frame Text|r")

    local tHpLabel = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    tHpLabel:SetPoint("TOPLEFT", targetTextHeader, "BOTTOMLEFT", 0, -10)
    tHpLabel:SetText("Health Bar Text:")

    local tHpDropdown = CreateSimpleDropdown("MarcusTargetHealthTextDropdown", contentFrame, "", "marcusTargetHealthText", textFormatOptions, function(val)
        BetterBlizzFramesDB.marcusTargetHealthText = val
        if BBF.RefreshMarcusTextLayout then BBF.RefreshMarcusTextLayout() end
    end, { anchorFrame = tHpLabel, x = 0, y = -14 }, 210)

    local tResLabel = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    tResLabel:SetPoint("TOPLEFT", tHpDropdown, "BOTTOMLEFT", 0, -12)
    tResLabel:SetText("Mana / Resource Bar Text:")

    local tResDropdown = CreateSimpleDropdown("MarcusTargetResourceTextDropdown", contentFrame, "", "marcusTargetResourceText", textFormatOptions, function(val)
        BetterBlizzFramesDB.marcusTargetResourceText = val
        if BBF.RefreshMarcusTextLayout then BBF.RefreshMarcusTextLayout() end
    end, { anchorFrame = tResLabel, x = 0, y = -14 }, 210)

    -- Text Size Sliders
    local sizeDivider = contentFrame:CreateTexture(nil, "ARTWORK")
    sizeDivider:SetPoint("TOPLEFT", pResDropdown, "BOTTOMLEFT", 0, -20)
    sizeDivider:SetSize(620, 1)
    sizeDivider:SetColorTexture(0.25, 0.25, 0.25, 0.5)

    local hpSizeSlider = CreateSlider(contentFrame, "Health Text Size", 8, 18, 1, "marcusTextFontSize", nil, 180)
    hpSizeSlider:SetPoint("TOPLEFT", sizeDivider, "BOTTOMLEFT", 10, -25)
    hpSizeSlider:HookScript("OnValueChanged", function()
        if BBF.RefreshMarcusTextLayout then BBF.RefreshMarcusTextLayout() end
    end)

    local resSizeSlider = CreateSlider(contentFrame, "Resource Text Size", 7, 16, 1, "marcusResourceFontSize", nil, 180)
    resSizeSlider:SetPoint("LEFT", hpSizeSlider, "RIGHT", 45, 0)
    resSizeSlider:HookScript("OnValueChanged", function()
        if BBF.RefreshMarcusTextLayout then BBF.RefreshMarcusTextLayout() end
    end)

    ---------------------------------------------------------------------------
    -- SECTION 2: HEALTH BAR FRAME COLORS
    ---------------------------------------------------------------------------
    local sec2Header = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    sec2Header:SetPoint("TOPLEFT", hpSizeSlider, "BOTTOMLEFT", -10, -40)
    sec2Header:SetText("|cffffd100HEALTH BAR FRAME COLORS|r")

    local sec2Desc = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    sec2Desc:SetPoint("TOPLEFT", sec2Header, "BOTTOMLEFT", 0, -4)
    sec2Desc:SetText("Pick a custom color changer or select from available default preset colors.")

    local playerColorOptions = {
        "Default",
        "Custom Color",
        "Class Color",
        "Green",
    ...
    -- Player Color Column
    local playerColorHeader = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalMed2")
    playerColorHeader:SetPoint("TOPLEFT", sec2Desc, "BOTTOMLEFT", 10, -18)
    playerColorHeader:SetText("|cff00c0ffPlayer Health Bar Color|r")

    local playerColorDropdown
    local playerColorBox = CreateColorBox(contentFrame, "marcusPlayerCustomColor", "Custom Color", function()
        BetterBlizzFramesDB.marcusPlayerColorMode = "Custom Color"
        if playerColorDropdown then
            playerColorDropdown:SetDefaultText("Custom Color")
        end
        if BBF.ApplyMarcusHealthColors then
            BBF.ApplyMarcusHealthColors()
        end
    end)
    playerColorBox:SetPoint("TOPLEFT", playerColorHeader, "BOTTOMLEFT", 0, -10)
    playerColorBox:SetScale(1.1)

    playerColorDropdown = CreateSimpleDropdown("MarcusPlayerColorDropdown", contentFrame, "Preset / Mode", "marcusPlayerColorMode", playerColorOptions, function(val)
        BetterBlizzFramesDB.marcusPlayerColorMode = val
        if BBF.ApplyMarcusHealthColors then
            BBF.ApplyMarcusHealthColors()
        end
    end, { anchorFrame = playerColorBox, x = 120, y = 5 }, 160)

    -- Target Color Column
    local targetColorHeader = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalMed2")
    targetColorHeader:SetPoint("TOPLEFT", sec2Desc, "BOTTOMLEFT", 340, -18)
    targetColorHeader:SetText("|cff00c0ffTarget Health Bar Color|r")

    local targetColorDropdown
    local targetColorBox = CreateColorBox(contentFrame, "marcusTargetCustomColor", "Custom Color", function()
        BetterBlizzFramesDB.marcusTargetColorMode = "Custom Color"
        if targetColorDropdown then
            targetColorDropdown:SetDefaultText("Custom Color")
        end
        if BBF.ApplyMarcusHealthColors then
            BBF.ApplyMarcusHealthColors()
        end
    end)
    targetColorBox:SetPoint("TOPLEFT", targetColorHeader, "BOTTOMLEFT", 0, -10)
    targetColorBox:SetScale(1.1)

    targetColorDropdown = CreateSimpleDropdown("MarcusTargetColorDropdown", contentFrame, "Preset / Mode", "marcusTargetColorMode", targetColorOptions, function(val)
        BetterBlizzFramesDB.marcusTargetColorMode = val
        if BBF.ApplyMarcusHealthColors then
            BBF.ApplyMarcusHealthColors()
        end
    end, { anchorFrame = targetColorBox, x = 120, y = 5 }, 160)

    -- Quick Presets Row for Player & Target
    local quickPresetLabel = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    quickPresetLabel:SetPoint("TOPLEFT", playerColorBox, "BOTTOMLEFT", 0, -32)
    quickPresetLabel:SetText("Quick Presets (Click to set instantly):")

    local presetsList = {"Green", "Red", "Bright Blue", "Yellow", "Cyan", "Purple", "Orange", "Dark Grey"}
    local presetColors = BBF.MarcusColorPresets or COLOR_PRESETS

    for i, pName in ipairs(presetsList) do
        local btn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
        btn:SetSize(68, 22)
        btn:SetPoint("TOPLEFT", quickPresetLabel, "BOTTOMLEFT", (i - 1) * 74, -8)
        btn:SetText(pName)
        local btnFont = btn:GetFontString()
        if btnFont then
            btnFont:SetFont(fontSmall, 10)
            local c = presetColors[pName]
            if c then
                btnFont:SetTextColor(c[1], c[2], c[3], 1)
            end
        end
        btn:SetScript("OnClick", function()
            BetterBlizzFramesDB.marcusPlayerColorMode = pName
            BetterBlizzFramesDB.marcusTargetColorMode = pName
            if playerColorDropdown then playerColorDropdown:SetDefaultText(pName) end
            if targetColorDropdown then targetColorDropdown:SetDefaultText(pName) end
            if BBF.ApplyMarcusHealthColors then BBF.ApplyMarcusHealthColors() end
        end)
    end

    ---------------------------------------------------------------------------
    -- Slider Helper for Marcus Options
    ---------------------------------------------------------------------------
    local function CreateMarcusSlider(parent, label, minVal, maxVal, stepVal, dbKey, width, onChange)
        local slider = CreateFrame("Slider", nil, parent, "OptionsSliderTemplate")
        slider:SetOrientation('HORIZONTAL')
        ...
        -- Decrement Button [ - ]
        local decBtn = CreateFrame("Button", nil, slider, "UIPanelButtonTemplate")
        decBtn:SetSize(18, 18)
        decBtn:SetPoint("LEFT", slider, "RIGHT", 8, 0)
        decBtn:SetText("-")
        local decFont = decBtn:GetFontString()
        if decFont then decFont:SetFont(fontSmall or GameFontNormalSmall:GetFont(), 11) end

        -- EditBox for direct numerical input & pixel-by-pixel precision
        local editBox = CreateFrame("EditBox", nil, slider, "InputBoxTemplate")
        editBox:SetSize(42, 18)
        editBox:SetPoint("LEFT", decBtn, "RIGHT", 4, 0)
        editBox:SetAutoFocus(false)
        editBox:SetFontObject(GameFontHighlightSmall)
        editBox:SetJustifyH("CENTER")

        -- Increment Button [ + ]
        local incBtn = CreateFrame("Button", nil, slider, "UIPanelButtonTemplate")
        incBtn:SetSize(18, 18)
        incBtn:SetPoint("LEFT", editBox, "RIGHT", 4, 0)
        incBtn:SetText("+")
        local incFont = incBtn:GetFontString()
        if incFont then incFont:SetFont(fontSmall or GameFontNormalSmall:GetFont(), 11) end
        ...
        local function UpdateValue(newVal, fromEditBox)
            ...
            if BetterBlizzFramesDB then
                BetterBlizzFramesDB[dbKey] = newVal
            end

            slider:SetValue(newVal)
            slider.Text:SetText(label)
            if not fromEditBox then
                editBox:SetText(FormatVal(newVal))
            end
        ...
        local initialValue = tonumber(BetterBlizzFramesDB and BetterBlizzFramesDB[dbKey]) or minVal
        slider:SetValue(initialValue)
        slider.Text:SetText(label)
        editBox:SetText(FormatVal(initialValue))
        ...
        -- Pixel-by-pixel mousewheel support: scrolling wheel increments/decrements 1 step
        ...
        slider.SetDisplayValue = function(self, val)
            UpdateValue(val, false)
        end
    ...
    ---------------------------------------------------------------------------
    -- SECTION 3: FRAME & RESOURCE BAR DIMENSIONS
    ---------------------------------------------------------------------------
    local sec3Header = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    sec3Header:SetPoint("TOPLEFT", quickPresetLabel, "BOTTOMLEFT", 0, -42)
    sec3Header:SetText("|cffffd100FRAME & RESOURCE BAR DIMENSIONS|r")

    local sec3Desc = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    sec3Desc:SetPoint("TOPLEFT", sec3Header, "BOTTOMLEFT", 0, -4)
    sec3Desc:SetText("Customize the width, height, and overall scale of the Player and Target frames and their resource bars.")

    -- Player Dimensions Column (Left)
    local pDimHeader = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalMed2")
    pDimHeader:SetPoint("TOPLEFT", sec3Desc, "BOTTOMLEFT", 10, -18)
    pDimHeader:SetText("|cff00c0ffPlayer Frame Dimensions|r")

    local pScaleSlider = CreateMarcusSlider(contentFrame, "Player Frame Scale", 0.60, 1.80, 0.05, "marcusPlayerFrameScale", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    pScaleSlider:SetPoint("TOPLEFT", pDimHeader, "BOTTOMLEFT", 0, -16)

    local pHpWidthSlider = CreateMarcusSlider(contentFrame, "Health Bar Width", 80, 260, 1, "marcusPlayerHealthWidth", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    pHpWidthSlider:SetPoint("TOPLEFT", pScaleSlider, "BOTTOMLEFT", 0, -22)

    local pHpHeightSlider = CreateMarcusSlider(contentFrame, "Health Bar Height", 10, 45, 1, "marcusPlayerHealthHeight", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    pHpHeightSlider:SetPoint("TOPLEFT", pHpWidthSlider, "BOTTOMLEFT", 0, -22)

    local pResWidthSlider = CreateMarcusSlider(contentFrame, "Resource Bar Width", 80, 260, 1, "marcusPlayerResourceWidth", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    pResWidthSlider:SetPoint("TOPLEFT", pHpHeightSlider, "BOTTOMLEFT", 0, -22)

    local pResHeightSlider = CreateMarcusSlider(contentFrame, "Resource Bar Height", 6, 30, 1, "marcusPlayerResourceHeight", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    pResHeightSlider:SetPoint("TOPLEFT", pResWidthSlider, "BOTTOMLEFT", 0, -22)

    -- Target Dimensions Column (Right)
    local tDimHeader = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalMed2")
    tDimHeader:SetPoint("TOPLEFT", sec3Desc, "BOTTOMLEFT", 340, -18)
    tDimHeader:SetText("|cff00c0ffTarget Frame Dimensions|r")

    local tScaleSlider = CreateMarcusSlider(contentFrame, "Target Frame Scale", 0.60, 1.80, 0.05, "marcusTargetFrameScale", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    tScaleSlider:SetPoint("TOPLEFT", tDimHeader, "BOTTOMLEFT", 0, -16)

    local tHpWidthSlider = CreateMarcusSlider(contentFrame, "Health Bar Width", 80, 260, 1, "marcusTargetHealthWidth", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    tHpWidthSlider:SetPoint("TOPLEFT", tScaleSlider, "BOTTOMLEFT", 0, -22)

    local tHpHeightSlider = CreateMarcusSlider(contentFrame, "Health Bar Height", 10, 45, 1, "marcusTargetHealthHeight", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    tHpHeightSlider:SetPoint("TOPLEFT", tHpWidthSlider, "BOTTOMLEFT", 0, -22)

    local tResWidthSlider = CreateMarcusSlider(contentFrame, "Resource Bar Width", 80, 260, 1, "marcusTargetResourceWidth", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    tResWidthSlider:SetPoint("TOPLEFT", tHpHeightSlider, "BOTTOMLEFT", 0, -22)

    local tResHeightSlider = CreateMarcusSlider(contentFrame, "Resource Bar Height", 6, 30, 1, "marcusTargetResourceHeight", 180, function()
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
    end)
    tResHeightSlider:SetPoint("TOPLEFT", tResWidthSlider, "BOTTOMLEFT", 0, -22)

    -- Reset Dimensions Button
    local resetDimsBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    resetDimsBtn:SetSize(160, 22)
    resetDimsBtn:SetPoint("TOPLEFT", pResHeightSlider, "BOTTOMLEFT", 0, -18)
    resetDimsBtn:SetText("Reset Frame Dimensions")
    local resetDimsFont = resetDimsBtn:GetFontString()
    if resetDimsFont then resetDimsFont:SetFont(fontSmall, 10) end
    resetDimsBtn:SetScript("OnClick", function()
        if InCombatLockdown() then return end
        BetterBlizzFramesDB.marcusPlayerHealthWidth = 124
        BetterBlizzFramesDB.marcusPlayerHealthHeight = 19
        BetterBlizzFramesDB.marcusPlayerResourceWidth = 124
        BetterBlizzFramesDB.marcusPlayerResourceHeight = 10
        BetterBlizzFramesDB.marcusPlayerFrameScale = 1.0

        BetterBlizzFramesDB.marcusTargetHealthWidth = 124
        BetterBlizzFramesDB.marcusTargetHealthHeight = 19
        BetterBlizzFramesDB.marcusTargetResourceWidth = 124
        BetterBlizzFramesDB.marcusTargetResourceHeight = 10
        BetterBlizzFramesDB.marcusTargetFrameScale = 1.0

        pScaleSlider:SetDisplayValue(1.0)
        pHpWidthSlider:SetDisplayValue(124)
        pHpHeightSlider:SetDisplayValue(19)
        pResWidthSlider:SetDisplayValue(124)
        pResHeightSlider:SetDisplayValue(10)

        tScaleSlider:SetDisplayValue(1.0)
        tHpWidthSlider:SetDisplayValue(124)
        tHpHeightSlider:SetDisplayValue(19)
        tResWidthSlider:SetDisplayValue(124)
        tResHeightSlider:SetDisplayValue(10)

        if BBF.ApplyMarcusDimensions then
            BBF.ApplyMarcusDimensions()
        end
    end)

    ---------------------------------------------------------------------------
    -- SECTION 4: PLAYER BUFFS POSITIONING (BELOW PLAYER FRAME)
    ---------------------------------------------------------------------------
    local sec4Header = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    sec4Header:SetPoint("TOPLEFT", resetDimsBtn, "BOTTOMLEFT", 0, -32)
    sec4Header:SetText("|cffffd100PLAYER BUFFS POSITIONING (BELOW PLAYER FRAME)|r")

    local sec4Desc = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    sec4Desc:SetPoint("TOPLEFT", sec4Header, "BOTTOMLEFT", 0, -4)
    sec4Desc:SetText("Display and position all your player buffs directly underneath the player frame.")

    local buffsCheckbox = CreateCheckbox("marcusBuffsBelowPlayer", "Show Buffs Below Player Frame", contentFrame, nil, function(opt, val)
        if BBF.UpdateMarcusBuffPosition then
            BBF.UpdateMarcusBuffPosition()
        end
    end)
    buffsCheckbox:SetPoint("TOPLEFT", sec4Desc, "BOTTOMLEFT", 10, -12)

    local debuffsCheckbox = CreateCheckbox("marcusDebuffsBelowBuffs", "Anchor Debuffs Below Buffs", contentFrame, nil, function(opt, val)
        if BBF.UpdateMarcusBuffPosition then
            BBF.UpdateMarcusBuffPosition()
        end
    end)
    debuffsCheckbox:SetPoint("TOPLEFT", buffsCheckbox, "BOTTOMLEFT", 0, -6)

    local hideCollapseCheckbox = CreateCheckbox("marcusBuffsHideCollapse", "Hide Buff Collapse Button (<)", contentFrame, nil, function(opt, val)
        if BBF.UpdateMarcusBuffPosition then
            BBF.UpdateMarcusBuffPosition()
        end
    end)
    hideCollapseCheckbox:SetPoint("TOPLEFT", debuffsCheckbox, "BOTTOMLEFT", 0, -6)

    local hideDurationCheckbox = CreateCheckbox("marcusBuffsHideDuration", "Hide Buff Duration Numbers", contentFrame, nil, function(opt, val)
        if BBF.UpdateMarcusBuffPosition then
            BBF.UpdateMarcusBuffPosition()
        end
    end)
    hideDurationCheckbox:SetPoint("TOPLEFT", hideCollapseCheckbox, "BOTTOMLEFT", 0, -6)

    -- Buff Position Sliders
    local buffsXSlider = CreateMarcusSlider(contentFrame, "Buffs Horizontal Offset (X)", -300, 300, 1, "marcusBuffsXOffset", 150, function()
        if BBF.UpdateMarcusBuffPosition then BBF.UpdateMarcusBuffPosition() end
    end)
    buffsXSlider:SetPoint("TOPLEFT", hideDurationCheckbox, "BOTTOMLEFT", 0, -22)

    local buffsYSlider = CreateMarcusSlider(contentFrame, "Buffs Vertical Offset (Y)", -300, 100, 1, "marcusBuffsYOffset", 150, function()
        if BBF.UpdateMarcusBuffPosition then BBF.UpdateMarcusBuffPosition() end
    end)
    buffsYSlider:SetPoint("TOPLEFT", buffsXSlider, "BOTTOMLEFT", 0, -22)

    local buffsScaleSlider = CreateMarcusSlider(contentFrame, "Buffs Icon Scale", 0.40, 2.50, 0.05, "marcusBuffsScale", 150, function()
        if BBF.UpdateMarcusBuffPosition then BBF.UpdateMarcusBuffPosition() end
    end)
    buffsScaleSlider:SetPoint("TOPLEFT", hideDurationCheckbox, "BOTTOMLEFT", 330, -22)

    local buffsPerRowSlider = CreateMarcusSlider(contentFrame, "Buffs Per Row", 2, 24, 1, "marcusBuffsPerRow", 150, function()
        if BBF.UpdateMarcusBuffPosition then BBF.UpdateMarcusBuffPosition() end
    end)
    buffsPerRowSlider:SetPoint("TOPLEFT", buffsScaleSlider, "BOTTOMLEFT", 0, -22)

    -- Reset Buffs Button
    local resetBuffsBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    resetBuffsBtn:SetSize(160, 22)
    resetBuffsBtn:SetPoint("TOPLEFT", buffsYSlider, "BOTTOMLEFT", 0, -18)
    resetBuffsBtn:SetText("Reset Buff Position")
    local resetBuffsFont = resetBuffsBtn:GetFontString()
    if resetBuffsFont then resetBuffsFont:SetFont(fontSmall, 10) end
    resetBuffsBtn:SetScript("OnClick", function()
        if InCombatLockdown() then return end
        BetterBlizzFramesDB.marcusBuffsXOffset = 0
        BetterBlizzFramesDB.marcusBuffsYOffset = -4
        BetterBlizzFramesDB.marcusBuffsScale = 0.85
        BetterBlizzFramesDB.marcusBuffsPerRow = 8
        BetterBlizzFramesDB.marcusBuffsHideCollapse = true
        BetterBlizzFramesDB.marcusBuffsHideDuration = false

        buffsXSlider:SetDisplayValue(0)
        buffsYSlider:SetDisplayValue(-4)
        buffsScaleSlider:SetDisplayValue(0.85)
        buffsPerRowSlider:SetDisplayValue(8)
        hideCollapseCheckbox:SetChecked(true)
        hideDurationCheckbox:SetChecked(false)
    ...
    ---------------------------------------------------------------------------
    -- Section 5: Target Buffs Positioning & Scaling
    ---------------------------------------------------------------------------
    local sec5Header = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    sec5Header:SetPoint("TOPLEFT", resetBuffsBtn, "BOTTOMLEFT", 0, -32)
    sec5Header:SetText("|cffffd100TARGET BUFFS POSITIONING & SCALING|r")

    local sec5Desc = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    sec5Desc:SetPoint("TOPLEFT", sec5Header, "BOTTOMLEFT", 0, -4)
    sec5Desc:SetText("High-quality square box scaling and pixel-by-pixel positioning for target buffs.")

    local targetSquareCheckbox = CreateCheckbox("marcusTargetBuffsSquare", "Square High-Quality Buffs (Match Player Frame)", contentFrame, nil, function(opt, val)
        if BBF.UpdateMarcusTargetBuffPosition then
            BBF.UpdateMarcusTargetBuffPosition()
        end
        if BBF.UpdateMarcusTargetBuffDurations then
            BBF.UpdateMarcusTargetBuffDurations()
        end
    end)
    targetSquareCheckbox:SetPoint("TOPLEFT", sec5Desc, "BOTTOMLEFT", 10, -12)

    local targetHideDurationCheckbox = CreateCheckbox("marcusTargetBuffsHideDuration", "Hide Buff Duration Numbers", contentFrame, nil, function(opt, val)
        if BBF.UpdateMarcusTargetBuffPosition then
            BBF.UpdateMarcusTargetBuffPosition()
        end
        if BBF.UpdateMarcusTargetBuffDurations then
    ...
    targetHideDurationCheckbox:SetPoint("TOPLEFT", targetSquareCheckbox, "BOTTOMLEFT", 0, -6)

    -- Target Buff Position Sliders
    local targetBuffsXSlider = CreateMarcusSlider(contentFrame, "Buffs Horizontal Offset (X)", -300, 300, 1, "marcusTargetBuffsXOffset", 150, function()
        if BBF.UpdateMarcusTargetBuffPosition then BBF.UpdateMarcusTargetBuffPosition() end
    end)
    targetBuffsXSlider:SetPoint("TOPLEFT", targetHideDurationCheckbox, "BOTTOMLEFT", 0, -22)

    local targetBuffsYSlider = CreateMarcusSlider(contentFrame, "Buffs Vertical Offset (Y)", -300, 100, 1, "marcusTargetBuffsYOffset", 150, function()
        if BBF.UpdateMarcusTargetBuffPosition then BBF.UpdateMarcusTargetBuffPosition() end
    end)
    targetBuffsYSlider:SetPoint("TOPLEFT", targetBuffsXSlider, "BOTTOMLEFT", 0, -22)

    local targetBuffsScaleSlider = CreateMarcusSlider(contentFrame, "Buffs Icon Scale", 0.40, 2.50, 0.05, "marcusTargetBuffsScale", 150, function()
        if BBF.UpdateMarcusTargetBuffPosition then BBF.UpdateMarcusTargetBuffPosition() end
    end)
    targetBuffsScaleSlider:SetPoint("TOPLEFT", targetHideDurationCheckbox, "BOTTOMLEFT", 330, -22)

    local targetBuffsPerRowSlider = CreateMarcusSlider(contentFrame, "Buffs Per Row", 2, 24, 1, "marcusTargetBuffsPerRow", 150, function()
        if BBF.UpdateMarcusTargetBuffPosition then BBF.UpdateMarcusTargetBuffPosition() end
    end)
    targetBuffsPerRowSlider:SetPoint("TOPLEFT", targetBuffsScaleSlider, "BOTTOMLEFT", 0, -22)

    -- Reset Target Buffs Button
    local resetTargetBuffsBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    resetTargetBuffsBtn:SetSize(160, 22)
    resetTargetBuffsBtn:SetPoint("TOPLEFT", targetBuffsYSlider, "BOTTOMLEFT", 0, -18)
    resetTargetBuffsBtn:SetText("Reset Target Buffs")
    local resetTargetBuffsFont = resetTargetBuffsBtn:GetFontString()
    if resetTargetBuffsFont then resetTargetBuffsFont:SetFont(fontSmall, 10) end
    resetTargetBuffsBtn:SetScript("OnClick", function()
        if InCombatLockdown() then return end
        BetterBlizzFramesDB.marcusTargetBuffsXOffset = 0
        BetterBlizzFramesDB.marcusTargetBuffsYOffset = -4
        BetterBlizzFramesDB.marcusTargetBuffsScale = 0.85
        BetterBlizzFramesDB.marcusTargetBuffsPerRow = 8
        BetterBlizzFramesDB.marcusTargetBuffsSquare = true
        BetterBlizzFramesDB.marcusTargetBuffsHideDuration = false

        targetBuffsXSlider:SetDisplayValue(0)
        targetBuffsYSlider:SetDisplayValue(-4)
        targetBuffsScaleSlider:SetDisplayValue(0.85)
        targetBuffsPerRowSlider:SetDisplayValue(8)
        targetSquareCheckbox:SetChecked(true)
        targetHideDurationCheckbox:SetChecked(false)
    ...
    ---------------------------------------------------------------------------
    -- SECTION 6: SAVE & APPLY SETTINGS
    ---------------------------------------------------------------------------
    local sec6Header = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    sec6Header:SetPoint("TOPLEFT", resetTargetBuffsBtn, "BOTTOMLEFT", 0, -28)
    sec6Header:SetText("|cffffd100SAVE & APPLY SETTINGS|r")

    local sec6Desc = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    sec6Desc:SetPoint("TOPLEFT", sec6Header, "BOTTOMLEFT", 0, -4)
    sec6Desc:SetText("Locks in your custom offsets, scales, dimensions, colors, and buff positioning. Settings are persisted across sessions.")

    local bottomSaveBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    bottomSaveBtn:SetSize(180, 28)
    bottomSaveBtn:SetPoint("TOPLEFT", sec6Desc, "BOTTOMLEFT", 0, -14)
    bottomSaveBtn:SetText("Save Settings")
    local bottomSaveFont = bottomSaveBtn:GetFontString()
    if bottomSaveFont then
        bottomSaveFont:SetFont(fontSmall or GameFontNormalSmall:GetFont(), 11)
        bottomSaveFont:SetTextColor(0, 1, 0, 1)
    end

    local bottomReloadBtn = CreateFrame("Button", nil, contentFrame, "UIPanelButtonTemplate")
    bottomReloadBtn:SetSize(180, 28)
    bottomReloadBtn:SetPoint("LEFT", bottomSaveBtn, "RIGHT", 16, 0)
    bottomReloadBtn:SetText("Save & Reload UI")
    local bottomReloadFont = bottomReloadBtn:GetFontString()
    if bottomReloadFont then
        bottomReloadFont:SetFont(fontSmall or GameFontNormalSmall:GetFont(), 11)
        bottomReloadFont:SetTextColor(1, 0.82, 0, 1)
    end

    local saveStatusText = contentFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    saveStatusText:SetPoint("LEFT", bottomReloadBtn, "RIGHT", 16, 0)
    saveStatusText:SetText("")

    local function PerformSave(reload)
        if InCombatLockdown() then
            BBF.Print("|cffff2020Cannot save settings while in combat!|r")
            return
        end

        -- Ensure all updates run
        if BBF.ApplyMarcusHealthColors then BBF.ApplyMarcusHealthColors() end
        if BBF.UpdateMarcusPlayerHealthText then BBF.UpdateMarcusPlayerHealthText() end
        if BBF.UpdateMarcusPlayerResourceText then BBF.UpdateMarcusPlayerResourceText() end
        if BBF.UpdateMarcusTargetHealthText then BBF.UpdateMarcusTargetHealthText() end
        if BBF.UpdateMarcusTargetResourceText then BBF.UpdateMarcusTargetResourceText() end
        if BBF.ApplyMarcusDimensions then BBF.ApplyMarcusDimensions() end
        if BBF.UpdateMarcusBuffPosition then BBF.UpdateMarcusBuffPosition() end
        if BBF.UpdateMarcusTargetBuffPosition then BBF.UpdateMarcusTargetBuffPosition() end
        if BBF.UpdateMarcusTargetBuffDurations then BBF.UpdateMarcusTargetBuffDurations() end
        if BBF.auraHosts and BBF.auraHosts.target and BBF.AnchorAuraContainer then
            BBF.AnchorAuraContainer(BBF.auraHosts.target)
        end
        if BBF.RestyleAuraButtons then BBF.RestyleAuraButtons(true) end

        if reload then
            ReloadUI()
        else
            if saveStatusText then
                saveStatusText:SetText("|cff00ff00[ Settings Saved & Applied ]|r")
                C_Timer.After(3, function()
                    if saveStatusText then saveStatusText:SetText("") end
                end)
            end
            BBF.Print("|A:gmchat-icon-blizz:16:16|a |cff00c0ffMarcus Custom Options|r: |cff00ff00Settings successfully saved & applied!|r")
        end
    end

    topSaveBtn:SetScript("OnClick", function() PerformSave(false) end)
    topReloadBtn:SetScript("OnClick", function() PerformSave(true) end)
    bottomSaveBtn:SetScript("OnClick", function() PerformSave(false) end)
    bottomReloadBtn:SetScript("OnClick", function() PerformSave(true) end)
