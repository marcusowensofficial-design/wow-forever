...
local function GetMarcusBorderCfg(unit, barType, defaultCfg)
    local db = BetterBlizzFramesDB
    if not db then return defaultCfg end
    local w, h
    if unit == "player" then
        if barType == "health" or barType == "healthBig" then
            w = db.marcusPlayerHealthWidth
            h = db.marcusPlayerHealthHeight
        elseif barType == "mana" then
            w = db.marcusPlayerResourceWidth
            h = db.marcusPlayerResourceHeight
        end
    elseif unit == "target" then
        if barType == "health" then
            w = db.marcusTargetHealthWidth
            h = db.marcusTargetHealthHeight
        elseif barType == "mana" then
            w = db.marcusTargetResourceWidth
            h = db.marcusTargetResourceHeight
        end
    end
    if w or h then
        return {
            width = w or defaultCfg.width,
            height = h or defaultCfg.height,
            startX = defaultCfg.startX,
            startY = defaultCfg.startY,
        }
    end
    return defaultCfg
end
...
local function GetPlayerHealthBorder()
    local base = BetterBlizzFramesDB and BetterBlizzFramesDB.bigPlayerHealthbar and BorderPositions.player.healthBig or BorderPositions.player.health
    return GetMarcusBorderCfg("player", "health", base)
end
...
local function MakeNoPortraitMode(frame)
    ...
    if frame == TargetFrame or frame == FocusFrame then
        ...
        if db.noPortraitPixelBorder then
            ...
            if frame == TargetFrame then
                local cfg = GetMarcusBorderCfg("target", "health", BorderPositions.target.health)
                BlackBorder(hpContainer.HealthBar, cfg.width, cfg.height, cfg.startX, cfg.startY)
                cfg = GetMarcusBorderCfg("target", "mana", BorderPositions.target.mana)
    ...
    elseif frame == PlayerFrame then
        ...
        if db.noPortraitPixelBorder then
            local cfg = GetPlayerHealthBorder()
            BlackBorder(hpContainer.HealthBar, cfg.width, cfg.height, cfg.startX, cfg.startY)
            cfg = GetMarcusBorderCfg("player", "mana", BorderPositions.player.mana)
            BlackBorder(manaBar, cfg.width, cfg.height, cfg.startX, cfg.startY)
